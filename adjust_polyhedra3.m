...
ref_stn = ['OHI2'; 'AUTF'; 'PALM'; 'FALK'; 'PARC'; 'RIO2'; 'KOUR'; 'POVE'];
...
for j = 1:5
    ...
    for i = 1:size(poly.epochs,1)
        ...
        L = [Lx(i,ref_index)' - X(i,ref_index)'; Ly(i,ref_index)' - Y(i,ref_index)'; Lz(i,ref_index)' - Z(i,ref_index)'];

        A = [Ax(ref_index,:); Ay(ref_index,:); Az(ref_index,:)];
